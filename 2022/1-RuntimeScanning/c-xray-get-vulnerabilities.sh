...
apikey=...
user=...
curl -u $user:$apikey -X POST $URL -H "Content-Type: application/json" -d @xray-query.json > c-vulnerabilities.json

# Format the vulnerabilities into just a list of impacted artifacts
cat c-vulnerabilities.json |  jq -c '.violations[].impacted_artifacts[]' | sort | uniq >> c-impacted-artifacts.txt

sed -i 's/\"//g' c-impacted-artifacts.txt

#Remove the leading `default/` in the `impacted_artifacts``
awk '{gsub("default/","");print}' <<< cat c-impacted-artifacts.txt >> c-formatted-impacted-artifacts
# There's also a trailing slash for whatever reason at the end get rid of it
sed -i 's/.$//' c-formatted-impacted-artifacts
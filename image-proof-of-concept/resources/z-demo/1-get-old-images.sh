...
curl -u myuser:$1 -X POST $URL -H "content-type: text/plain" -d @1-old-images-request.txt >> 1-artifactory-get.json 

cat 1-artifactory-get.json | jq -c '.results[] | {completepath: "\(.repo)/\(.path)"}' |
while read -r line
do
